...
aStarNode = class "aStarNode" {
	...
	addPort = function(this, oNode)--TODO add map and layer as args since the port could resonably go to another map/layer
...
F = math.clamp(B + Naf * M, ASTAR_NODE_ENTRY_COST_MIN, ASTAR_NODE_ENTRY_COST_MAX);]]
	...
	isPassable = function(this, ...)
		...
		--if this is query not invloving rovers, depend only on the node's passability (which is true here)
		bRet = bRet or #tOutRovers > 0;
